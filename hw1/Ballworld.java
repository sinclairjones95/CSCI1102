...
public class Ballworld extends JFrame // appears in a GUI window
   ...
   Ballworld() 
   {
      ...
      JButton addStrippedButton = new JButton("Add StrippedBall"); //new botton - Stripped ball
      buttonPanel.add(addStrippedButton);
      ...
      addBlinkButton.addActionListener(new ActionListener(){       //adding a blinking ball
       ...
       addStrippedButton.addActionListener(new ActionListener(){       //adding a Stripped ball
         public void actionPerformed(ActionEvent e){
            // instantiate a Ball
            Ball Strippedball = new StrippedBall(drawingPanel.getWidth(),drawingPanel.getHeight());
            // the ball responds to timer ticks
            timer.addActionListener(Strippedball);
            // add it to the Ball list
            ballList.add(Strippedball);
         }
      });
   ...
   void handleCollisions(){
     for(Ball b : ballList){      //check collisions for all balls
       ...
       for (Ball c: ballList){     //nested loop to to compare each ball with all other balls
         boolean intersect = b.intersect(c);    //calling the intersect method between the two balls compared
         if (intersect==true){             //if they do intersect, call the collide method
          b.collide(c);
         }
         else {
           continue;
